# Chocolatey package for ShortGeek

The package source for `shortgeek` on the Chocolatey community repository lives here so that
`packageSourceUrl` in the nuspec points at something real.

## What the package does

It downloads `ShortGeekSetup.exe` from the GitHub release for the matching tag and verifies it against a
...
2. Take the SHA-256 for `ShortGeekSetup.exe` from the release's own `SHA256SUMS.txt`.
...
5. Install locally from the built nupkg and check the shim works:
   `choco install shortgeek -s . -y`
6. `choco push shortgeek.<version>.nupkg --source https://push.chocolatey.org/`
